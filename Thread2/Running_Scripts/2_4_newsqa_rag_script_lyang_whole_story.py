        ...
        for dist_function in dist_functions:
            ...
            for top_n_sentence in top_n_sentences:
                ...
                for i, story in enumerate(data['data']):
                    ...
                    # Process the entire story text
                    story_text = story['text']

                    # Embed the whole story text instead of individual sentences
                    story_emb = hf_story_embs.embed_documents([story_text])[0]
                    ...
                    # Initialize the QA chain (without a retriever, since you're using the whole story)
                    qa_chain = RetrievalQA.from_chain_type(
                        llm, 
                        retriever=None,  # No retriever needed as we use the whole story
                        chain_type="stuff",
                        verbose=False,
                        chain_type_kwargs={
                    ...
                    for j, question_data in enumerate(story['questions']):
                        ...
                        # Form the context for QA using the whole story
                        context_for_qa = story_text
