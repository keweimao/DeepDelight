...
def newsqa_loop(data, llm, output_csv_path, output_log_path, max_stories,
    ...
    with open(output_csv_path, 'w', newline='') as file:
        ...
        writer.writerow(['Chunk_size', 'Chunk_Overlap', 'Dist_Func', 'Top_N', 'Time', 'Story Number', 'Question Number', 'EM', 'Precision', 'Recall', 'F1', 'Error'])
                ...
                for dist_function in dist_functions:
                    ...
                    for top_n_sentence in top_n_sentences:
                        ...
                        for i, story in enumerate(data['data']):
                            ...
                            # Chunks splits for "sentences"
                            sentences = text_splitter.split_text(story['text'])
                            ...
                            for j, question_data in enumerate(story['questions']):
                                ...
                                if error==0: 
                                    writer.writerow([chunk_size, overlap_percentage, dist_function, top_n_sentence, time.time() - start_time, i, j, em_score, precision, recall, f1_score_value, error])
