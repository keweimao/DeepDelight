        ...
        for top_n_sentence in top_n_sentences:
            ...
            for cov in combined_data:
                try:
                    ...
                    answers_pairs.append((cov['answers'],predict,chunk_size,overlap_percentage,top_n_sentence,func))
                except Exception as e:
                    print(f"Error processing {cov}: {e}. Skipping to the next item.")
...
for i in answers_pairs:
    ...
    #print()
    if predicted_answer=="error prompt size exce context window size process":
        continue
    else:
        result_list.append([i[4],i[5],i[2],i[3],i[1]['query'],predict_answer,actual_answer,f1_score,precision,recall])

df=pd.DataFrame({'top_n_sentences':[],'dist_function':[],'Chunk_size':[],'Overlap_percentage':[],'Quesion':[],'Predicted Answer':[],'Actual Answer':[],'f1 score':[],'Precision':[],'Recall':[]})
