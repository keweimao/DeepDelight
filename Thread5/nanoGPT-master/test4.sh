...
output_file="training_times_test4.txt"
...
# Train the model
python train_a2r0_4.py config/train_shakespeare_char_a2.py
...
min=$(( cost_time / 60 ))
sec=$(( cost_time % 60 ))
echo "Training time for a2r0_4: $min min $sec s" >> "$output_file"

# Repeat the above steps for other training scripts

# For train_a2r2.py
start_time=$(date +%s)
python train_a2r2_4.py config/train_shakespeare_char_a2.py
end_time=$(date +%s)
cost_time=$(( end_time - start_time ))
min=$(( cost_time / 60 ))
sec=$(( cost_time % 60 ))
echo "Training time for a2r2_4: $min min $sec s" >> "$output_file"

# For train_a2r5.py
start_time=$(date +%s)
python train_a2r5_4.py config/train_shakespeare_char_a2.py
end_time=$(date +%s)
cost_time=$(( end_time - start_time ))
min=$(( cost_time / 60 ))
sec=$(( cost_time % 60 ))
